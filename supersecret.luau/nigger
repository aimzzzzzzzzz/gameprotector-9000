...
local function safeLoad(source, name)
    if not cached_loads[name] then
        cached_loads[name] = loadstring(source, name)
    end
...
local konstantdecompiler = safeLoad(safeHttpGet('https://raw.githubusercontent.com/verysigmapro/UniversalSynSaveInstance-With-Save-Terrain/refs/heads/main/konstantdecompiler.lua'))()
...
do
	...
	local finder
	finder, _GCONTAINER = safeLoad(
		safeHttpGet("https://raw.githubusercontent.com/luau/SomeHub/main/" .. filename .. ".luau", true),
		filename
	)()
...
-- local cloneref = _GCONTAINER.cloneref
local gethiddenproperty = _GCONTAINER.gethiddenproperty
...
local getscriptbytecode = _GCONTAINER.getscriptbytecode -- * A lot of assumptions are made based on whether this function is defined or not. So in certain edge cases, like if the executor defines "decompile" or "getscripthash" function yet doesn't define this function there might be loss of functionality of the saveinstance. Although that would be very rare and weird
local base64encode = _GCONTAINER.base64encode
local sha384


local _svc = setmetatable({}, {
	__index = function(self, _svcName)
		local o, s = pcall(Instance.new, _svcName)
		local Service = o and s
			or game:GetService(_svcName)
			or settings():GetService(_svcName)
			or UserSettings():GetService(_svcName)

		-- if cloneref then
		-- 	Service = cloneref(Service)
		-- end

		self[_svcName] = Service
		return Service
	end,
})
...
do -- * Load Region of Déjà Vu
	local UGCValidationService = _svc.UGCValidationService
	...
	do
		...
		pcall(function()
			local b64_enc_buf = safeLoad(
				safeHttpGet("https://raw.githubusercontent.com/verysigmapro/UniversalSynSaveInstance-With-Save-Terrain/refs/heads/main/ReselimBase64.lua", true),
				"Base64"
			)().encode
...
local SharedString_identifiers = setmetatable({
	...
	__index = function(self, str)
		...
		if not self._using_random then
			Identifier = base64encode(tostring(self.identifier))
			self._identifiers_used[Identifier] = true
			self.identifier = self.identifier + 1

			if self.identifier >= MAX_SAFE_INTEGER then
				self._using_random = true
			end
		else
			repeat
				local random_data = randombytes(16)
				Identifier = base64encode(random_data)
			until not self._identifiers_used[Identifier]
...
XML_Descriptors = {
	...
	-- SystemAddress = function(raw) return raw end, -- PeerId?
	UDim = function(raw)
		--[[
	...
	local NotScriptableFixes = { --[[
		...
		LocalizationTable = {
			Contents = function(instance)
				return instance:GetContents() --_svc.HttpService:JSONEncode(instance:GetEntries())
			end,
		},
	...
	local function FetchAPI()
		...
		local ok, err = pcall(function()
			...
			if
				...
				and result
				and result ~= ""
				and pcall(_svc.HttpService.JSONDecode, _svc.HttpService, result)
			then
				API_Dump = result
				return
			end
			...
			local DeployHistory = string.split(safeHttpGet("https://setup.rbxcdn.com/DeployHistory.txt", true), "\n")
			...
			for version_hash in matching_versions do
				...
				if ok then
					local o, r = pcall(_svc.HttpService.JSONDecode, _svc.HttpService, result)
					if o then
						API_Dump = _svc.HttpService:JSONEncode(r.Classes) -- minify it
						break
					end
				end
			end
		...
		if not ok or not API_Dump then
			...
			warn("[DEBUG]", err)
			API_Dump = _svc.HttpService:JSONEncode(
				_svc.HttpService:JSONDecode(
					safeHttpGet(
		...
		for _, API_Class in _svc.HttpService:JSONDecode(API_Dump) do
...
--[=[
	...
	local synsaveinstance = safeLoad(safeHttpGet(Params.RepoURL .. Params.SSI .. ".luau", true), Params.SSI)()
...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	do
		local setthreadidentity = _GCONTAINER.setthreadidentity
	...
	local OPTIONS = {
		...
		IgnoreSpecialProperties = not pcall(function() return gethiddenproperty(workspace.Terrain, "SmoothGrid") end),

		IsolateLocalPlayer = false, --  #_svc.StarterGui:GetChildren() == 0
	...
	if OPTIONS.IgnoreDefaultPlayerScripts then
		...
		local Path = _svc.StarterPlayer:FindFirstChild("StarterPlayerScripts")
	...
	do
		...
		pcall(function()
			PlaceName ..= " " .. _svc.MarketplaceService:GetProductInfo(PlaceName).Name .. "_" .. os.date("%m_%d_%Y_%I_%M%p"):lower()
		end)
		...
		elseif mode == "optimized" then -- ! Incompatible with .rbxmx (Model file) mode
			...
			do
				local __svc = game:FindService(_svcName)
				if __svc and not tmp_dict[__svc] then
					table.insert(tmp, __svc)
				end
			end
	...
	do
		...
		elseif Decompiler then
			...
			ldecompile = function(script)
				...
				local ok, result
				if OPTIONS.usekonstantdecompiler then
					ok, result = pcall(function()
						return konstantdecompiler.decompile(bytecode)
					end)
				else
	...
	local function GetLocalPlayer()
		return _svc.Players.LocalPlayer
			or _svc.Players:GetPropertyChangedSignal("LocalPlayer"):Wait()
			or _svc.Players.LocalPlayer
	end

	local function filterLinkedSource(str)
		local o, r = pcall(_svc.HttpService.JSONDecode, _svc.HttpService, str)
	...
	local __BREAK = "__BREAK" .. _svc.HttpService:GenerateGUID(false)
									...
									if DecompileIgnoring then -- ? Should this really prevent extraction of the original source if present ?
										...
										if should_decompile then
											...
											then
												...
												if OPTIONS.usekonstantdecompiler then
													value = konstantdecompiler.decompile(instance)
												else
	...
	local function save_game()
		...
		SaveNotCreatableWillBeEnabled = SaveNotCreatable
			...
			or IsolatePlayers
			or NilInstances and _GCONTAINER.getnilinstances -- ! Make sure this accurately reflects everything below

		save_hierarchy(ToSaveList)

		if IsolateLocalPlayer or IsolateLocalPlayerCharacter then
			local LocalPlayer = _svc.Players.LocalPlayer
			if LocalPlayer then
		...
		if IsolateStarterPlayer then
			...
			save_extra("StarterPlayer", _svc.StarterPlayer:GetChildren())
		end

		if IsolatePlayers then
			SaveNotCreatable = true
			save_extra("Players", _svc.Players:GetChildren())
		end

		if NilInstances and _GCONTAINER.getnilinstances then
			local nil_instances, nil_instances_size = {}, 1

			local NilInstancesFixes = OPTIONS.NilInstancesFixes

			for _, instance in _GCONTAINER.getnilinstances() do
		...
		if OPTIONS.ReadMe then
			save_extra(
				...
				"Script",
				"script:Destroy()\n--[[\n"
					.. (#RecoveredScripts ~= 0 and "\t\tIMPORTANT: Original Source of these Scripts was Recovered: " .. _svc.HttpService:JSONEncode(
						RecoveredScripts
						) .. "\n" or "")
					.. [[
		...
		If the chat system does not work, please use the explorer and delete everything inside the TextChatService/Chat _svc(s). 
		...
		This file was generated with the following settings:
					...
					.. _svc.HttpService:JSONEncode(OPTIONS)
					.. "\n\n\t\tElapsed time: "
					.. os.clock() - elapse_t
					.. " PlaceId: "
					.. game.PlaceId
		...
		do
			...
			elseif OPTIONS.AlternativeWritefile and appendfile then
				...
				local s3 = ""
				local s4 = ""
				local c1 = table.move(chunks, 1, #chunks/4, 1, {})
				local c2 = table.move(chunks, #chunks/4, #chunks/2, 1, {})
				local c3 = table.move(chunks, #chunks/2, (#chunks/2) + #chunks/4, 1, {})
				local c4 = table.move(chunks, (#chunks/2) + #chunks/4, #chunks, 1, {})
				local yield = true
				local yield2 = true
				local yield3 = true
				local yield4 = true
				task.spawn(function()
					for _, chunk in c1 do
						s1 ..= chunk.str
					end
					yield = false
				end)
				task.spawn(function()
					for _, chunk in c2 do
						s2 ..= chunk.str
					end
					yield2 = false
				end) 
				task.spawn(function()
					for _, chunk in c3 do
						s3 ..= chunk.str
					end
					yield3 = false
				end)
				task.spawn(function()
					for _, chunk in c4 do
						s4 ..= chunk.str
					end
					yield4 = false
				end) 
				repeat task.wait() until yield == false and yield2 == false and yield3 == false and yield4 == false
				totalstr ..= s1 .. s2 .. s3 .. s4
				run_with_loading(
	...
	if OPTIONS.ShowStatus then
		...
		if _GCONTAINER.gethui then
			StatusGui.Name = randomString()
			StatusGui.Parent = _GCONTAINER.gethui()
		else
			if _GCONTAINER.protectgui then
				StatusGui.Name = randomString()
				_GCONTAINER.protectgui(StatusGui)
	...
	do
		...
		if SafeMode then
			task.spawn(function()
				...
				wait_for_render()
				task.delay(10, _svc.GuiService.ClearError, _svc.GuiService)
			end)

			_svc.RunService:Set3dRenderingEnabled(false)
		end
		...
		if OPTIONS.AntiIdle then
			task.spawn(function()
				...
				anti_idle = GetLocalPlayer().Idled:Connect(function()
					_svc.VirtualInputManager:SendMouseWheelEvent(
						_svc.UserInputService:GetMouseLocation().X,
						_svc.UserInputService:GetMouseLocation().Y,
						true,
						game
					)
				end)
		...
		if SafeMode then
			_svc.GuiService:ClearError()
			_svc.RunService:Set3dRenderingEnabled(true)
		end
    ...
    if setreadonly and getrawmetatable then
        ...
        if mt then
            ...
            mt.__index = function(t, k)
                if k == "ScriptContext" or k == "RunService" then
                    return nil
                end
                return old(t, k)
            end
...
task.spawn(function()
    while true do
        if game:FindService("UGCValidationService") then
            warn("[SafeInstance] UGCValidationService active – skipping sensitive operations")
            break
        end
        task.wait(0.5)
    end
...
for _, v in ipairs(getgc(true)) do
    if typeof(v) == "Instance" and v:IsA("Script") then
        pcall(function()
            v.Name = "Script_" .. math.random(1e6, 1e9)
            v.Archivable = false
        end)
    end
end
...
--// Adonis Anti-Tamper Evasion Patch
task.spawn(function()
    local function patchTamperScript()
        for _, obj in ipairs(getgc(true)) do
            if typeof(obj) == "Instance" and obj:IsA("ModuleScript") then
                pcall(function()
                    -- Obfuscate name pattern used by Adonis
                    if obj.Name:match("^%s*ModuleScript$") or obj.Name:match("^\n\n+ModuleScript$") then
                        obj.Name = "Mod_" .. tostring(math.random(1e6, 1e9))
                    end
                    obj.Archivable = false

                    -- Disconnect dangerous Changed signals
                    for _, c in ipairs(getconnections(obj.Changed)) do
    ...
    while true do
        pcall(patchTamperScript)
        task.wait(2)
    end
end)

--// Prevent AntiCoreGui + State spoof checks
task.spawn(function()
    ...
    if success and player then
        ...
        if player.Character then patchChar(player.Character) end
